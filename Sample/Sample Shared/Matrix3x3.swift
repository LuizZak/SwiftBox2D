...
public struct Matrix3x3: CustomStringConvertible {
    ...
    /// translation to the vector.
    @inlinable
    public func transformPoint(_ vec: B2Vec2) -> B2Vec2 {
        ...
        if result.z != 0 && result.z != 1 {
            return B2Vec2(x: result.x, y: result.y) / result.z
        }

        return B2Vec2(x: result.x, y: result.y)
    }
    ...
    /// vector.
    @inlinable
    public func transformVector(_ vec: B2Vec2) -> B2Vec2 {
        let vec3 = Vector3(vec, z: 1)
    ...
    @inlinable
    public static func make2DScale(_ vec: B2Vec2) -> Self {
    ...
    /// according to the specified amounts.
    @inlinable
    public static func make2DTranslation(_ vec: B2Vec2) -> Self {
